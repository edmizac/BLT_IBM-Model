...
filename <- here("Model_development", "Model-cleaning", "runtime", "v1_output_2022-05-11d", "tempRDS.Rdata")
nl <- readRDS(filename)
...
gp3 <- gua.sf +
  ...
  scale_color_manual(values = simulated_colors) +
  scale_shape_manual(values = simulated_shapes) +
  ggtitle("Output map of 10 d runs for each random-seed")
gp3

# ggsave(filename = here("Model_development", "Model-cleaning", "runtime", "v1_output_2022-05-11d", "Output-map-of-10d-runs-for-each-random-seed.png"),
#        dpi = 300, width = 30, height = 20, units = "cm")
...
dat.all$datetime <- as.character(dat.all$datetime) 
...
dpl <- function(df) {
  ...
  dist_list <- data.frame(Group = "", DPL = -1, Day = "", Month = "")
  dist <- numeric()
  for (group in unique(df$id)) {
    df3 <- df[df$id == group, ]
    for (df4 in split(df3, df3$dd_mm_yyyy)) {
      dist <- 0
      for (i in 2:nrow(df4)) {
        dist <- dist + sqrt((df4$x[i] - df4$x[i - 1]) ^ 2 +
                              (df4$y[i] - df4$y[i - 1]) ^ 2)
      }
      dist_list <- rbind.data.frame(dist_list, 
                                    c(group, dist, df4$dd_mm_yyyy[1], as.character(df4$month[1])),
...
```{r}
dpl_all %>% 
  ggplot(aes(x = Group, y = DPL, color = Group)) +
  geom_boxplot() +
  guides(fill=FALSE) +
...
plot(k95_sim_gua_sf[k95_sim_gua_sf$id == "Guareí_-1062115611", ])
...
dat.summary <- dat.all %>% 
  ...
  dplyr::select(c(Group, behavior, activity_budget)) %>% 
  unique() %>% 
  ungroup()
...
Plot activity budget ALL behaviors
```{r}
dat.summary %>% 
  ggplot(aes(x = Group, y = activity_budget, fill = behavior)) +
