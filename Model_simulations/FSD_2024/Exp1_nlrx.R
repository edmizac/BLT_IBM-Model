...
# Script purpose: Run the v1.2 Model with the imported build-forest landscapes:  
# Experiment 1 - full design FOR FRUGIVORES & SEED DISPERSAL SYMPOSIA 2024

# Date created: 2024-07-09d
...
## Packages -------------------------
library("nlrx")
# library("here") # here does not work if I source the code through the R base
library("dplyr")
library("stringr")
library("progressr")
library("future")
...
param_table <- read.csv("Data/Parameter_table.csv",
...
# files_forests <- list.files(pathfiles, pattern = ".rds")
# files_forests <- paste0(pathfiles, "/", files_forests)
files_forests <- stringr::str_remove(files_forests, ".csv")
# files_forests <- stringr::str_remove(files_forests, ".rds")

# n files should be 15760, but it is 2674:
...
length(files_forests) + length(files_done) # it should be = 2674
...
# db <- bind_rows(db, results) # this made the loop crash once
...
# print info in the end of the simulation
...
# filename <- paste0(outpath, "size1000shapefact23104.05_R1.02_p0.734_NN136.77random.rds")
# nl2 <- readRDS(filename)
# nl2 <- readRDS(paste0(outpath, "/without_SDD/", "size150shapefact1381.91_R1_p0.934_NN121.07random.rds"))
# nl2 <- getsim(nl2, "simoutput")
# nl2 %>% glimpse()
