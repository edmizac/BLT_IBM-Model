...
;      set pcolor white
  ...
  if setup-mode = "random" [
    ...
    repeat lc-patch-size [
      ask blob-maker [
        ask min-one-of patches with [ allowed? = TRUE ] [ distance myself ] [
          set lc-val 1
          set pcolor green
        ]
        rt random 360
  ...
  if setup-mode = "patch-shape-1982" [
    ...
    while [ count patches with [ pcolor = green ]  < lc-patch-size ] [
      ask blob-maker [
        ask min-one-of patches with [ allowed? = TRUE ] [ distance myself ] [
          ;        if any? turtles with [ pcolor != white ] [
          ;          ask patch-here [
        ...
        ;          ]
        ;        ]
;        if patch-ahead pcolor = white
        if [allowed?] of (patch-set patch-here neighbors) = FALSE
        [ right 180 forward 0.1 ]
