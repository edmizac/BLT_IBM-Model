...
;    let breaks-interval []
    ...
    let breaks-list n-values lc-patch-size [0] ; list of 0's
    let breaks-n round ( lc-patch-size / n-dendro ) ; index of breaks, e.g. 25
;    let breaks-interval n-values ( length breaks-list ) [ x -> x + breaks-n ]
    let breaks-interval lput breaks-n
    print map [ i -> breaks-n * i ] [ 1 2 3 ]

;    let breaks-list-dir []

    set breaks-list ( remove-item breaks-interval breaks-list )
    set breaks-list ( insert-item breaks-interval breaks-list ( one-of direction1-list ) )
...
;      ifelse count patches with [ pcolor = green ] < lc-patch-size / n-dendro (
;
;      )
