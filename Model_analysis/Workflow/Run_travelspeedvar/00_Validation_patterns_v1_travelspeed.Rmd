...
dat.gua.orig <- read.csv2("D:/Data/Documentos/github/BLT-Movement-Patterns/asltraj/Previous_2022-05-19d/ltraj_df_Data_Guarei_Felipe_EMZ1.csv", dec = ".", stringsAsFactors = TRUE)
...
dat.gua.orig <- read.csv2("D:/Data/Documentos/github/BLT-Movement-Patterns/asltraj/Previous_2022-05-19d/Data_Guarei_FelipeBufalo_EMZ1.csv",
...
dat.sma.orig <- read.csv2(here("D:/Data/Documentos/github/BLT-Movement-Patterns/asltraj/Previous_2022-05-19d/ltraj_df_Data_Sma_EMZ5.csv"), 
                          dec = ".", stringsAsFactors = TRUE)
dat.suz.orig <- read.csv2(here("D:/Data/Documentos/github/BLT-Movement-Patterns/asltraj/Previous_2022-05-19d/Data_Suzano_Anne_EMZ4.csv"), 
                          dec = ".", stringsAsFactors = TRUE)
dat.taq.orig <- read.csv2(here("D:/Data/Documentos/github/BLT-Movement-Patterns/asltraj/Previous_2022-05-19d/Data_PEMDTaquara_Anne_EMZ4.csv"), 
                          dec = ".", stringsAsFactors = TRUE)
...
dpl_all_summary <- dpl_all %>% 
  dplyr::group_by(Group, TravelSpeed) %>% 
  summarise(mean = mean(DPL), sd = sd(DPL), n = n())

dpl_all_summary

a <- flextable(dpl_all_summary)
path_output <- here("Model_analysis", "Workflow", "Run_travelspeedvar",
                       "DPL_comparisons.html")
save_as_html(
  a,
  path = path_output)
...
dpl_all_summary_month <- dpl_all %>% 
  dplyr::group_by(Group, Month, TravelSpeed) %>% 
  summarise(mean = mean(DPL), sd = sd(DPL), n = n())

dpl_all_summary_month
...
hr_values_summary <- hr_values %>% 
  dplyr::group_by(Group, TravelSpeed) %>% 
  summarise(mean_95 = mean(KDE_95), sd_95 = sd(KDE_95), 
            mean_50 = mean(KDE_50), sd_50 = sd(KDE_50), 
            n = n())

a <- flextable(hr_values_summary)
...
# ggsave(filename = here("Model_analysis", "Workflow", "Run_travelspeedvar",
#                        "ActivityBudget_4behaviors-others.png"),
#        dpi = 300, width = 30, height = 20, units = "cm")
```
