...
```{r sim_out}
nl.out <- readRDS("morris_2022-08-04_phenology-off_small.rds")
sim_out <- nl.out@simdesign@simoutput
monkeys <- data.table::rbindlist(sim_out$metrics.monkeys)
monkeys$agent <- NULL
monkeys$dist_traveled <- monkeys$`dist-traveled` * 28.29 # (0.7 vel = 19.80162 m)
...
```
```{r combinations}
run_comb <- unique(dt_bs[ , .(step_forget, visual, #trees_reset, 
                              p_foraging, duration)])
setkey(run_comb, cols = "step_forget", "visual", #"trees_reset",
       "p_foraging"
                      , "duration")
run_comb
```
