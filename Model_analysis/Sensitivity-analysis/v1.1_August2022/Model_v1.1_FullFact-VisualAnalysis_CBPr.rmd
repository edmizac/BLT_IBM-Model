...
4. Parameters (inputs) that showed consistent trend on variables (outputs): 
  + step forget on HR & DPL (inverse relationship)
  + p foraging on DPL (inverse relationship)
  + duration on DPL (inverse relationship)
...
nl_off_s <- readRDS(here("Model_analysis/Sensitivity-analysis/temp/Model_v1.1_Sensitivity_FullFact_2022-08-04d/", "morris_2022-08-04_phenology-off_small.rds"))
data_off_s <- unnest_simoutput(nl_off_s) ; rm(nl_off_s)

data_off_s <- data_off_s %>% 
  mutate(feedingbout = "Feeding bout off")

nl_on_s <- readRDS(here("Model_analysis/Sensitivity-analysis/temp/Model_v1.1_Sensitivity_FullFact_2022-08-04d/", "morris_2022-08-04_phenology-on_small.rds"))
...
ptable <- cbind("step_forget", paste(sprintf("%4.2f", val_step_forget)
                               , collapse = ", "))
...
db_nest <- db_nest %>% 
  dplyr::select(-c(3, 4)) # does not work by collumn name
...
db %>%
  ...
  # xlab("DPL (m)") +
  xlab("Deslocamento diário (m)") +
  # ggtitle("Effect of feeding bout on DPL") +
  ggtitle("Efeito do 'feedingbout' no deslocamento diário") +
...
db %>%
  ...
  ggtitle("Efeito do 'feedingbout' na área de vida (KDE 95%)") +
...
p1 <- db %>% 
  ...
  ylab("Deslocamento diário (m)") +
  xlab("step_revisit") +
  geom_boxplot(width = 0.4) +
  # ggtitle("Effect of step_forget on DPL") +
  ggtitle("Efeito do 'step_revisit' no deslocamento diário") +
...
p2 <- db %>% 
  ...
  ggtitle("Efeito de 'step_revisit' na área de vida (KDE 95%)") +
  xlab("step_revisit") +
  theme(legend.position="none")
...
p3 <- db %>% 
  ...
  ggtitle("Efeito do 'visual' no deslocamento diário")+
  theme(legend.position="none")
  # facet_wrap(~feedingbout)
...
p4 <- db %>% 
  ...
  ggtitle("Efeito do 'visual' na área de vida (KDE 95%)") +
  theme(legend.position="none")
...
p5 <- db %>% 
  ...
  xlab("prob. forrageio") +
  geom_boxplot(width = 0.4) +
  ggtitle("Efeito da 'prob. forrageio' no deslocamento diário")+
  theme(legend.position="none")
  # facet_wrap(~feedingbout)
...
p6 <- db %>% 
  ...
  geom_boxplot() +
  xlab("prob. forrageio") +
  # ggtitle("Effect of p_foraging on Home range (KDE 95%)") +
  ggtitle("Efeito da 'prob. forrageio' na área de vida (KDE 95%)") +
  theme(legend.position="none")
...
p8 <- db %>% 
  ...
  # facet_wrap(~feedingbout) +
  geom_boxplot() +
  ggtitle("Effect of duration on home range (KDE 95%)")
...
Up to know I have plotted all the simulations in relation to one unique variable. Now, we will plot runs while keeping the other variables constant
