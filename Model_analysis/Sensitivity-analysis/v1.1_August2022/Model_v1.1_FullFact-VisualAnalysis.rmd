...
db$id %>% unique()
a <- db %>% filter(id == "GuaSim_1150" | id == "GuaSim_5513") 
a <- db %>% filter(id == "GuaSim_190") 
...
plot(KDE) 
plot(mcp, add.relocations = FALSE, add = TRUE, lty = 2)    
...
a2 <- a %>%
  mutate(
    ...
    # datetime = case_when(levels(id) == 1)
...
db_nest_a2 <- a2 %>% 
  mutate( 
    ...
    KDE50 = amt::map(data, ~ hr_kde(., level = 0.95))
  )
...
a1 <- a %>%
  make_track(.x=x, .y=y, id = id, crs = our_crs) #, all_cols = TRUE) #%>% nest(data = -c(id))
...
mov2 <- mov2 %>% 
  summarise(
    step_length_mean = mean(step_length, na.rm = TRUE),
    step_length_sd = sd(step_length, na.rm = TRUE),
    turn_ang_mean = circular::mean.circular(turn_ang, na.rm = TRUE),
    turn_ang_sd = circular::sd.circular(turn_ang, na.rm = TRUE)
