...
if(Sys.info()[["nodename"]] == "DESKTOP-R12V3D6") {
  ...
  outpath <- here("Model_analysis", "Sensitivity-analysis", "v1.1_2023Jan", "Param_bestguess", "temp")
  user_scp = "\"Eduardo\""
}
...
# for (i in i:nrow(param.table)) {
  ...
  # Loop or choose one area/month
  area_run_scp <- '"Guareí"' #scaped
  month_run_scp <- '"May"'   #scaped
  
  # area_run <- "Guareí" #non scaped
  # month_run <- "Jun"   #non scaped
...
# }
  ...
  # expname = paste0("v1.1_Morris_", area_run, month_run)
...
nl@experiment <- experiment(expname = expname,
                            ...
                            variables = list(
                              ...
                              "energy_stored_val" = list(min = 0, max = 2000, qfun="qunif"), # THIS IS THE MAIN DIFFERENCE FROM MODEL V1.1 TO V1.2
...
nseeds <- 6  #4  # (= num repetitions -> match with ncores)
...
nl_on <- readRDS(file.path(paste0(outpath, "/"
                             , "v1.1_Morris_Guareí_Jun_Feedingbout_on_2023-03-31.rds")))
