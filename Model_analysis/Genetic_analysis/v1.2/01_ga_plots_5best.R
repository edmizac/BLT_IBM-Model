...
for (i in filesga) {
  ...
  # bestSolution<-resultsrbga$population[which.min(resultsrbga$evaluations), ]
  ...
  # # best5 <- cbind(fitness, idx)
  # best5 <- best5 %>% 
  #   as_tibble() %>% 
  #   slice_min(fitness, n = 5, with_ties = FALSE)
  # 
  # best5
  ...
  best5 <- resultsrbga$best[resultsrbga$best %in% best5] #%>% unique() ; 
  length(best5_idx)
...
# Save plot
ggsave(paste0(path,  "/with_stored_energy/", '01_GA_optimized-params-feedingbout-on_wrap2-best5.png'), 
       height = 5, width = 10)
...
# Save plot
# ggsave(paste0(path,  "/with_stored_energy/", '01_GA_optimized-params_feedingbout-on3_connected-fitness.png'), height = 5, width = 7)
