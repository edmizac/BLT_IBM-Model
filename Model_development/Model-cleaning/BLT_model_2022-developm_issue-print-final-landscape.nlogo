...
to move-monkeys
  ...
  [
    ...
    if timestep > simulation-time [ ; if the day is over  ; MAYARA CODE WAS SENDING TAMARINS TO SLEEPING-TREES ONLY IN THE LAST STEP
...
; export the landscape as a .png if neccessary  (= Milles et al 2020)
  if day = no_days AND max-output = TRUE [
    let file-id random 999999999
    let world-name (word runtime (remove ":" remove "" remove " " remove "." date-and-time) "_" "e-" start-energy file-id "_world.png")
