...
;--------------------------------------------------------------------------------
to setup-world
  import-world "BLT_model_v1.11-world.csv"
end
...
;  set foraging_speed foraging_speed_val
  ...
  if (energy_level_2 < energy_level_1) [
    error "Energy Level 2 has to be greater than Energy Level 1"
    stop
  ]
  ...
  ;; BLT ROUTINE
    ...
    if timestep < simulation-time [ ; energy levels: energy_level_1 = 80 and energy_level_2 = 150
      ifelse energy < energy_level_1 [ ; energy < level 1
        set travel_mode "short_distance" ;; 1
        set ld_tree_target -1
        frugivory
      ][
;        ifelse (action = "feeding" or action = "travel") [   ;; v1.0 version
        ifelse (travel_mode = "short_distance" ) [                       ;; v1.1 version (FOR MAKING THEY FORAGE WHILE TRAVELLING)
          ifelse energy > energy_level_2 [ ; energy > level 2 ==> other activities
            set travel_mode "long_distance" ;; 2
            set tree_target -1
            set tree_current -1
...
;;      ifelse random (2 * species_time) > action-time [
      ifelse random (species_time * energy_level_1 / energy) > action-time [
        feeding
      ][
        set tree_current -1
        to-feeding-tree
