...
;; GLOBALS ;;
globals [
  behaviorsequence ; for the activity budget plot
...
to setup-monkeys
  ...
  ask monkeys [
    ...
    set behaviorsequence [] ; initialize behavior sequence list for the activity budget
...
;;    set shape "banana"
    ...
    set action-time 0
    set action "travel"
    set behavior ""        ; none of the first scans in each day has the behavior
...
to move-monkeys
  ...
  ask monkeys
  [
;    while [ patch-ahead pcolor = yellow + 4] [
;    ask patch-here [ set pcolor red]
;    rt 180 fd 1
;  ] ; no use of the matrix
...
to feeding
  ...
  set behaviorsequence lput 1 behaviorsequence ;; activity budget. 1 = feeding
...
to to-feeding-tree
  ...
  ; random noise in directionality while traveling (=Hopkins 2016)
  rt (random 60) - 30 ; angle higher than "(random 90) - 45" makes them lose too much energy
  ...
  set behaviorsequence lput 3 behaviorsequence ; activity budget. 3 = travel
  ]
...
;      set tree_add_list [] ;; COMMENT OUT THIS BECAUSE THE TAMARINS DON'T FORGET
  ...
  if action != "sleeping" [
    ...
    ; random noise in directionality while traveling (=Hopkins 2016)
    rt (random 45) - 22.5 ; angle higher than "(random 90) - 45" makes them lose too much energy
    ...
    set behaviorsequence lput 3 behaviorsequence ; activity budget. 3 = travel (to sleeping site in this case)
  ]
...
;---------------------------------------------------------------------------------------------
to forage
  set color magenta ; to see in which part of the route tamarins are foraging


;  if [pcolor] of patch-ahead 1 = yellow + 4 [
;    ask monkeys [ set pcolor black]
;    rt 180 fd 1
;  ] ; no use of the matrix
  ...
  set behaviorsequence lput 2 behaviorsequence ; activity budget. 2 = foraging
...
;  ]
  ...
  set color grey ; to set color of tamarins back to grey after forage behavior was finished

end


to travel ; not being called
  forward travel_speed
  set behavior "travel"
  set steps-moved steps-moved + 1
  set energy energy + energy-loss-traveling
